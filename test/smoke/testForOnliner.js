...
const { checkFolders, visualTest } = require('../../utils/resembleTesting');
...
describe("Catalog page for TV", function() {
    ...
    it("Price text fields of the left panel filter shoud allow entering numbers only", async function() {
        ...
        expect(receivedText).to.eql(processedValue);
    });

    it("Visual testing of the left panel", async function() {
        this.timeout(120000);
        const sample = {
            filename: 'catalogTvLeftPanel', 
            boundingBox: { left: 252, top: 360, right: 504, bottom: 3251 }
        };

        await checkFolders();
        const misMatchPercentage = parseFloat( await visualTest(page, sample) );
        if (misMatchPercentage <= 100) {
            expect(misMatchPercentage).to.be.below(1);
        } else {
            console.log('Original image was created. Run the test one more time.');
        }
    });
});